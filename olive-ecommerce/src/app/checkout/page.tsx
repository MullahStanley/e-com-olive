...
import { CreditCard, Smartphone, MapPin, Loader2, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  // New state to handle the M-Pesa waiting UX
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (paymentMethod === 'mpesa' && data.mpesaStatus !== 'failed') {
        // Trigger the waiting modal for M-Pesa users
        setIsWaitingForMpesa(true);
        
        // Wait 5 seconds to let them look at their phone before redirecting
        setTimeout(() => {
          router.push(`/track-order?order=${data.orderNumber}`);
        }, 5000);
      } else {
        // Instant redirect for card payments or fallback
      ...
      setLoading(false); // Only stop loading if there's an error (otherwise we redirect)
    } 
  };
    ...
    <>
      ...
      {isWaitingForMpesa && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
          <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full text-center animate-in zoom-in-95 duration-300">
            <div className="relative w-20 h-20 mx-auto mb-6">
              <div className="absolute inset-0 bg-green-100 rounded-full animate-ping opacity-75"></div>
              <div className="relative bg-green-500 text-white w-20 h-20 rounded-full flex items-center justify-center shadow-lg">
                <Smartphone size={36} />
              </div>
            </div>
            <h2 className="text-2xl font-extrabold text-gray-900 mb-2">Check your phone</h2>
            <p className="text-gray-600 mb-6 leading-relaxed">
              We've sent an M-Pesa prompt to your phone. Please enter your PIN to complete the payment.
            </p>
            <div className="flex items-center justify-center text-sm text-gray-500 font-medium space-x-2">
      ...
      <div className={`bg-gray-50 min-h-screen py-12 ${isWaitingForMpesa ? 'pointer-events-none overflow-hidden' : ''}`}>
